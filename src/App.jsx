...
function App() {
  ...
  // 1. Listen to Firebase for ANY payment (Coin Voucher or RFID)
  useEffect(() => {
    ...
    const unsubscribe = onValue(paymentRef, (snap) => {
      ...
      if (data) {
        ...
        // Auto-process if it's an RFID tap (No typing needed!)
  ...
  // 3. Handle RFID Auto-Connect
  const handleRfidConnect = (data) => {
    setConnectionStatus("connected");
    // RFID gives a standard 30 mins (adjust as needed) or fetch from user profile
    const timeToAdd = 30 * 60; 
    setActiveTimeSeconds(prev => prev + timeToAdd);
    setUsedPayments(prev => [...prev, data.card_id]);
  };

  // 4. Handle Manual Voucher Connect (Stacking)
  const handleConnect = () => {
    ...
    if (validVoucher) {
      setConnectionStatus("connected");
      const timeToAdd = validVoucher.minutes * 60;
      setActiveTimeSeconds(prev => prev + timeToAdd);
            ...
            {latestPayment ? (
              <>
                <div style={styles.bigCode}>
                  {latestPayment.type === "COIN" ? latestPayment.code : "RFID TAP"}
                </div>
                <p>
                  {latestPayment.type === "COIN" 
                    ? `Voucher: ${latestPayment.minutes} Mins` 
                    : `Card ID: ${latestPayment.card_id}`}
                </p>
              </>
            ) : (
          ...
          <div style={styles.phoneScreen}>
            ...
            {connectionStatus === "connected" ? (
              <div style={styles.successScreen}>
                ...
                <p style={{ color: '#94a3b8' }}>Internet access enabled</p>
                ...
                <div style={styles.topUpSection}>
                  ...
                  />
                  <button style={{...styles.btn, backgroundColor: '#3b82f6'}} onClick={handleConnect}>
                    Add Time
                  </button>
                </div>
              </div>
              ...
              <div style={styles.loginScreen}>
                <p style={{ textAlign: 'center', color: '#94a3b8' }}>Tap your RFID card on the machine or enter a voucher code below.</p>
                <input 
                  style={styles.input} 
                  placeholder="VOUCHER" 
                  value={inputCode}
...
const styles = {
  ...
  phonePanel: { backgroundColor: '#000', padding: '12px', borderRadius: '35px', width: '300px', height: '550px' },
